...
class HtmlParser {
	...
	public function parse($url, $callback = null) {
		PhWeb::getStream($url)
			->setHeader('Accept', 'text/html, application/xhtml, */*; q=0.9')
			->addCallback(array($this, 'processChunk'))
			->execute();
	}
	
	public function processChunk($notification) {
		$url = $notification['location'];
		$this->buffers[$url] .= $notification['message'];
		if ($notification['code'] == 'connect') {
			$this->buffers[$url] = '';
		}
		if (($notification['code'] == 'receive') && $notification['headers']['_code'] == 200) {
